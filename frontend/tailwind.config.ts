...
function getAlpha(color: string, alphaKey: string) {
  return `color-mix(in srgb, ${color} ${parseInt(alphaKey)}%, transparent)`;
}
...
export default {
  ...
  theme: {
    extend: {
      colors: {
        background: {
          DEFAULT: getAlpha("var(--background)", "50"),
          full: "color-mix(in srgb, var(--background) 100%, #ffffff)"
        },
        backgroundSecondary: {
          DEFAULT: getAlpha("var(--backgroundSecondary)", "50"),
